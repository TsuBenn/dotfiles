...
def handle_normal_input(stdscr, key: int, state: ExplorerState, height: int, width: int):
...
def _handle_open(state: ExplorerState):
...
def run_explorer(stdscr, start_dir: str):
    curses.start_color()
    ...
    curses.init_pair(7, 23, -1)                    # Dimmed custom dark gray for hidden files
    ...
    while not state.should_quit:
        ...
        if state.in_menu_mode:
            ...
            handle_normal_input(stdscr, key, state, height, width)
...
def main():
    # Resolve optional starting directory argument
    start_dir = os.getcwd()
    if len(sys.argv) >= 2 and sys.argv[1]:
        expanded_path = os.path.expanduser(sys.argv[1])
    ...
    # Redirect stdout (fd 1) to stderr (fd 2) while curses runs.
    # This keeps the TUI rendering on the terminal window instead of dumping
    # ANSI escape codes into subshell command substitution $(miniex).
    saved_stdout_fd = os.dup(1)
    os.dup2(2, 1)

    try:
        chosen_file = curses.wrapper(run_explorer, start_dir)
    except KeyboardInterrupt:
        pass
    except Exception as exc:
        print(f"Fatal error: {exc}", file=sys.stderr)
    finally:
        # Restore original stdout for printing the selected path
        os.dup2(saved_stdout_fd, 1)
        os.close(saved_stdout_fd)

    # Print selected file path cleanly to stdout
    if chosen_file:
        print(chosen_file)
