...
Singleton {
    ...
    function playSound(file: string, vol: real) {
        playsound.exec(["bash","-c","paplay --volume="+ 65536*vol + " -d @DEFAULT_SINK@ .config/quickshell/old/assets/sfx/" + file + ".mp3"])
    }
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                for (const data of datas) {
                    ...
                    switch (name.trim()) {
                        case ".config/quickshell/old/assets/sfx/mambo.mp3": continue
                        case ".config/quickshell/old/assets/sfx/mambo_tongye.mp3": continue
                        case ".config/quickshell/old/assets/sfx/mambo_wow.mp3": continue
                    }
